define(["Q", "bridge/cordova"], function(Q, cordova) {
  ...
  var authenticate = function(ccoId, password) {
    ...
    var success = function(message) {
      console.log("native serivce - success callback");
      deffered.resolve(message);
    };
    var fail = function(error) {
      console.log("native serivce - Fail callback");
      console.error(error);
      deffered.reject(error);
    };
    console.log("native services - authenticate");
    cordova().exec(success, fail, "Authentication", "authenticate", [ccoId, password,device.uuid,device.model]);
